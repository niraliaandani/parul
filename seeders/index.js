...
async function seedUserRole (){
  try {
    let user = await User.findOne({
      'email':'Ewald.Daniel32@hotmail.com',
      'isActive':true,
      'isDeleted':false
    });
    let userRole = await Role.findOne({ code: 'SYSTEM_USER' }, { id: 1 });
    if (user && user.isPasswordMatch('2X077OD6Jfoxt36') && userRole){
    ...
    let admin = await User.findOne({
      'email':'Nikki82@yahoo.com',
      'isActive':true,
      'isDeleted':false
    });
    let adminRole = await Role.findOne({ code: 'SYSTEM_USER' }, { id: 1 });
    if (admin && admin.isPasswordMatch('ryo2MgNo_CjdMwp') && adminRole){
      let count = await UserRole.countDocuments({
        userId: admin.id,
        roleId: adminRole.id,
        isActive: true, 
...
async function seedUser () {
  try {
    ...
    if (!user || !user.isPasswordMatch('2X077OD6Jfoxt36') ) {
      let user = new User({
        'password':'2X077OD6Jfoxt36',
        'email':'Ewald.Daniel32@hotmail.com',
    ...
    if (!admin || !admin.isPasswordMatch('ryo2MgNo_CjdMwp') ) {
      let admin = new User({
        'password':'ryo2MgNo_CjdMwp',
        'email':'Nikki82@yahoo.com',
